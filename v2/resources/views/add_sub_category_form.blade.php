                     ...
                     <div class="card-body">
                         <div class="row">
                            <div class="col-md-12">
                              ...
                              <select class="form-control supcat-dropdown" name="super_category_id">
                                  @foreach($supcat as $row)
                                    <option value="{{$row->id}}" {{old ('super_category_id') == $row->id ? 'selected' : ''}}>{{$row->name}}</option>
                                  @endforeach
                              </select>
                            </div>
                        </div>
...
$('.supcat-dropdown').on('change', function() {
var id = $(this).val();
// alert(city_id);
$(".store-dropdown").html('');
$.ajax({
...
$(".category-dropdown").append('<option value="'+value.id+'">'+value.name+'</option>');
});
}
});
});
