...
class DropdownController extends Controller
{
    ...
    public function getAdminData(Request $request){
        ...
        $data['block']= DB::table('blocked_store_admin')->where("store_admin_id",$request->admin_id)->where('status','=',1)->count();
    ...
    public function getspecification(Request $request){
        $datas = DB::table('subcategory_restriction')->where("subcategory_id",$request->subcategory_id)->get(["restrictionspecification"]);
        foreach($datas as $specids){
            $prod='';
            if($specids!=''){
              $specidss=explode(',',$specids->restrictionspecification);
    		  foreach($specidss as $asd){
    		      if($asd!=''){
    		          $data=DB::table('specifications')->select('*')->where('name','=',$asd)->first();
    		          $data1=explode(',',$data->value);
    		           $prod.='<label>'.$asd.'</label><select class="form-control" name="'.$asd.'">';
    		           foreach($data1 as $dt){
    		               $prod.='<option value="'.$dt.'">'.$dt.'</option>';
    		           }
    		      }
    		  }
    		}
